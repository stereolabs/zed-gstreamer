...
void GLViewer::displayPointCloud(bool c)
{
   show_point_cloud_on_display = c;
}
...
void GLViewer::update() {
    ...
    if (keyStates_['d']  == KEY_STATE::UP )
        show_point_cloud_on_display = !show_point_cloud_on_display;
...
void GLViewer::draw() {
    ...
    if (show_point_cloud_on_display)
...
void GLViewer::reshapeCallback(int width, int height) {
    ...
    int side = fmax((float)width,(float)height);

    int vp_x = (width - side) / 2;
    int vp_y = (height - side) / 2;
    int vp_w = side;
    int vp_h = side;
