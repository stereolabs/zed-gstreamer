...
class PollEvent {
  public:
    PollEvent() { event_signaled = false; }
    ~PollEvent() {}
    inline void notify() {
    ...
    inline bool poll(int timeout_ms) {
        ...
        if (!event_signaled)
            isFinishedWaiting = cv_poll.wait_for(lk, std::chrono::milliseconds(timeout_ms),
                                                 [this] { return event_signaled; });
...
enum class AEANTIBANDING { OFF, AUTO, HZ50, HZ60 };
...
class __attribute__((visibility("default"))) ArgusVirtualCapture {
  public:
    ...
    virtual ARGUS_STATE openCamera(const ArgusCameraConfig &config,
                                   bool recreate_daemon = true) = 0;
    ...
    ///
    /// \brief enterCriticalSection: provide a mutex locking mechanism when using the buffer filled
    /// by the SDK
    ///
    ...
    ///
    /// \brief setPtr : Pass a memory pointer to the internal buffer so that memcpy is avoid when
    /// using getPixels() \note : getPixels is still required to be called \param ptr_sdk : must be
    /// at the exact size (width*height*channels)
    ///
...
class __attribute__((visibility("default"))) ArgusBayerCapture : public ArgusVirtualCapture {
  ...
  public:
    ...
    ///
    /// \brief setFrameExposureRange: set the Frame exposure range. Use that to set a fixed exposure
    /// (low == high) \param exp_low : min exposure time (in us) the sensor will not gow below
    /// \param exp_high : max exposure time (in us) the sensor will not gow higher \return 0 if
    /// success, <0 if camera unavailable, >0 if error from argus side
    ///
    int setFrameExposureRange(uint64_t exp_low, uint64_t exp_high);
    ...
    /// \param [out] min_exp : the returned minimum exposure time possible from the DTS
    /// configuration. Provided in microseconds \param [out] max_exp : the returned maximum exposure
    /// time possible from the DTS configuration. Provided in microseconds \return 0 if success, !=0
    /// otherwise
    ///
    int getExposureLimits(uint64_t &min_exp, uint64_t &max_exp);

    ///
    /// \brief setManualExposure : set Manual exposure for the specified camera (side) between 0
    /// (lowest exp) to 100 (highest exp) \param percent : [0-100] for exp range \return 0 if
    /// success, <0 if camera unavailable, >0 if error from argus side
    ///
    int setManualExposure(int percent);
    ...
    ///
    /// \brief setAnalogFrameGainRange : set the sensor gain (analog) range. Use that to set a fixed
    /// analog gain (low == high) \param gain_low : min gain the sensor will not gow below \param
    /// gain_high : max gain the sensor will not gow higher \return 0 if success, <0 if camera
    /// unavailable, >0 if error from argus side
    ///
    int setAnalogFrameGainRange(float gain_low, float gain_high);
    ...
    ///
    /// \brief setManualGain : set Gain for the specified camera (side) between 0 (lowest exp) to
    /// 100 (highest exp) \param percent : [0-100] for gain range \return 0 if success, <0 if camera
    /// unavailable, >0 if error from argus side
    ///
    int setManualAnalogGain(int percent);
    ...
    ///
    /// \brief setDigitalFrameGainRange : set the ISP gain (digital) range. Use that to set a fixed
    /// digital gain (low == high) \param gain_low : min gain the ISP will not gow below \param
    /// gain_high : max gain the ISP will not gow higher \return 0 if success, <0 if camera
    /// unavailable, >0 if error from argus side
    ///
    int setDigitalFrameGainRange(float gain_low, float gain_high);
    ...
    ///
    /// \brief setManualDigitalGain : set Digital Gain for the specified camera (side) between 0
    /// (lowest gain) to 100 (highest gain) \param percent : [0-100] for gain range \return 0 if
    /// success, <0 if camera unavailable, >0 if error from argus side
    ///
    int setManualDigitalGain(int percent);
  ...
  private:
    ...
    void unlock();
    int dump_dmabuf(int dmabuf_fd, unsigned int plane, argusMonoImage &image);
    void estimageRGGBGainFromColorTemperature(uint32_t KelvinT, float factor, float &r,
                                              float &g_even, float &g_odd, float &b);
    void estimageRGGBGainFromColorTemperature_v2(uint32_t KelvinT, float factor, float &r,
                                                 float &g_even, float &g_odd, float &b);
