...
ARGUS_STATE ArgusV4l2Capture::openCamera(const ArgusCameraConfig &config, bool recreate_daemon) {
    ...
    if (ctx.cam_fd == -1) {
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] Failed to open camera for " << mPort
    ...
    if (ioctl(ctx.cam_fd, VIDIOC_S_FMT, &fmt) < 0) {
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] Failed to SET camera output format for " << mPort
                      << " || Err : " << strerror(errno) << std::endl;
    ...
    if (ioctl(ctx.cam_fd, VIDIOC_G_FMT, &fmt) < 0) {
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] Failed to GET camera output format for " << mPort
                      << " || Err : " << strerror(errno) << std::endl;
    ...
    if (fmt.fmt.pix.width != ctx.cam_w || fmt.fmt.pix.height != ctx.cam_h ||
        ...
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] Warning -- Resolution requested not supported for Port"
                      << mPort << " : Switching to " << ctx.cam_w << " x " << ctx.cam_h
                      << std::endl;
    }
    ...
    if (mConfig.verbose_level > 0)
        std::cout << "[ArgusCapture] Port" << mPort << " : Resolution " << fmt.fmt.pix.width
                  << " x " << fmt.fmt.pix.height << "@"
                  << streamparm.parm.capture.timeperframe.denominator << "/"
                  << streamparm.parm.capture.timeperframe.numerator << std::endl;
    ...
    if (ctx.g_buff == NULL) {
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] Failed to allocate global buffer context for " << mPort
                      << std::endl;
    ...
    for (unsigned int index = 0; index < V4L2_BUFFERS_NUM; index++) {
        ...
        if (-1 == NvBufSurfaceFromFd(fd[index], (void **) (&pSurf))) {
            if (mConfig.verbose_level > 0)
                std::cout << "[ArgusCapture] Failed to get NvBuffer parameters for " << mPort
                          << std::endl;
    ...
    if (!requestCameraBuffers()) {
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] Failed to REQUEST camera buffers for " << mPort
                      << std::endl;
...
bool ArgusV4l2Capture::requestCameraBuffers() {
    ...
    if (ioctl(ctx.cam_fd, VIDIOC_REQBUFS, &rb) < 0) {
        printf("Failed to request v4l2 buffers: %s (%d)", strerror(errno), errno);
        return false;
    }
    ...
    for (unsigned int index = 0; index < V4L2_BUFFERS_NUM; index++) {
        ...
        if (ioctl(ctx.cam_fd, VIDIOC_QUERYBUF, &buf) < 0) {
            printf("Failed to query buff: %s (%d)", strerror(errno), errno);
            return false;
        }
        ...
        if (ioctl(ctx.cam_fd, VIDIOC_QBUF, &buf) < 0) {
            printf("Failed to enqueue buffers: %s (%d)", strerror(errno), errno);
            return false;
        }
    }
...
void ArgusV4l2Capture::closeCamera() { opened_ = false; }

ArgusV4l2Capture::~ArgusV4l2Capture() { closeCamera(); }

ARGUS_STATE ArgusV4l2Capture::reboot() { return ARGUS_STATE::UNSUPPORTED_FCT; }
    ...
    while (poll(fds, 1, 5000) > 0 && !exit_) {
        if (fds[0].revents & POLLIN) {
            ...
            if (ioctl(ctx.cam_fd, VIDIOC_DQBUF, &v4l2_buf) < 0)
                printf("Failed to dequeue camera buff: %s (%d)", strerror(errno), errno);

            ctx.frame++;

            NvBufSurface *pSurf = NULL;
            if (-1 == NvBufSurfaceFromFd(ctx.g_buff[v4l2_buf.index].dmabuff_fd, (void **) (&pSurf)))
            ...
            if (ctx.capture_dmabuf) {
                ...
                if (-1 == Raw2NvBufSurface(ctx.g_buff[v4l2_buf.index].start, 0, 0, ctx.cam_w,
                                           ctx.cam_h, pSurf))
                    printf("Cannot copy raw buffer to NvBufsurface plane");
            }

            /*  Convert the camera buffer from YUV422 to RGBA */
            if (NvBufSurf::NvTransform(&transform_params, ctx.g_buff[v4l2_buf.index].dmabuff_fd,
                                       mCvtfd))
            ...
            if (mConfig.verbose_level > 3)
                std::cout << "[ArgusCapture][Debug] --> New image available for port : " << mPort
                          << " Timestamp : " << newImage.imageTimestamp
            ...
            if (ioctl(ctx.cam_fd, VIDIOC_QBUF, &v4l2_buf))
                printf("Failed to queue camera buffers: %s (%d)", strerror(errno), errno);
        } else
            usleep(10);
    }
}
...
int ArgusV4l2Capture::dump_dmabuf(int dmabuf_fd, unsigned int plane, argusMonoImage &image) {
    //  if (dmabuf_fd <= 0)
    //    return -1;
...
uint64_t ArgusV4l2Capture::getFrameExposureTime() { return 0; }

int ArgusV4l2Capture::getFrameExposurePercent() { return -1; }

int ArgusV4l2Capture::setFrameExposureRange(uint64_t exp_low, uint64_t exp_high) { return -1; }

int ArgusV4l2Capture::getFrameExposureRange(uint64_t &exp_low, uint64_t &exp_high) { return -1; }

int ArgusV4l2Capture::getExposureLimits(uint64_t &min_exp, uint64_t &max_exp) { return -1; }

int ArgusV4l2Capture::setManualExposure(int percent) { return -1; }

int ArgusV4l2Capture::setManualTimeExposure(uint64_t time) { return -1; }

int ArgusV4l2Capture::setAutomaticExposure() { return -1; }
...
float ArgusV4l2Capture::getAnalogFrameGain() { return -1; }

int ArgusV4l2Capture::getAnalogFrameGainPercent() { return -1; }

int ArgusV4l2Capture::setAnalogFrameGainRange(float gain_low, float gain_high) { return -1; }

int ArgusV4l2Capture::getAnalogFrameGainRange(float &sgain_low, float &sgain_high) { return -1; }

int ArgusV4l2Capture::getAnalogGainLimits(float &min_gain, float &max_gain) { return -1; }

int ArgusV4l2Capture::setAutomaticAnalogGain() { return -1; }

int ArgusV4l2Capture::setManualAnalogGain(int percent) { return -1; }

int ArgusV4l2Capture::setManualAnalogGainReal(int db) { return -1; }
...
float ArgusV4l2Capture::getDigitalFrameGain() { return 0; }

int ArgusV4l2Capture::setDigitalFrameGainRange(float gain_low, float gain_high) { return -1; }

int ArgusV4l2Capture::getDigitalFrameGainRange(float &dgain_low, float &dgain_high) { return -1; }

int ArgusV4l2Capture::getDigitalGainLimits(float &min_gain, float &max_gain) { return -1; }

int ArgusV4l2Capture::setAutomaticDigitalGain() { return -1; }

int ArgusV4l2Capture::setManualDigitalGain(int percent) { return -1; }

int ArgusV4l2Capture::setManualDigitalGainReal(int fact) { return -1; }

///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////// White Balance(ISP) CONTROL/REQUEST
//////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
int ArgusV4l2Capture::setManualWhiteBalance(uint32_t color_temperature_) { return -1; }

uint32_t ArgusV4l2Capture::getManualWhiteBalance() { return -1; }

int ArgusV4l2Capture::setAutomaticWhiteBalance(int val) { return -1; }

int ArgusV4l2Capture::getAutomaticWhiteBalanceStatus() { return -1; }
...
int ArgusV4l2Capture::setSharpening(float value) { return -1; }

float ArgusV4l2Capture::getSharpening() { return 0.0; }

///////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////   Saturation(ISP) CONTROL/REQUEST
//////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
int ArgusV4l2Capture::setColorSaturation(float saturation) { return -1; }

float ArgusV4l2Capture::getColorSaturation() { return -1; }

///////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////   Exposure EV (ISP) CONTROL/REQUEST
////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
int ArgusV4l2Capture::setExposureCompensation(float ev) { return -1; }

float ArgusV4l2Capture::getExposureCompensation() { return 0.0; }

//////////////////////////////////
///////////AEaNtibanding//////////
//////////////////////////////////
int ArgusV4l2Capture::setAEAntiBanding(AEANTIBANDING mode) { return -1; }

///////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////   Denoiser (ISP) CONTROL/REQUEST
//////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
int ArgusV4l2Capture::setDenoisingValue(float value) { return -1; }

float ArgusV4l2Capture::getDenoisingValue() { return 0; }

///////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////   ToneMapping CONTROL/REQUEST
//////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
int ArgusV4l2Capture::getToneMappingCurveSize(int channel) { return -1; }

int ArgusV4l2Capture::setToneMappingFromGamma(int channel, float gamma) { return -1; }

float ArgusV4l2Capture::getGamma() { return 0; }

///////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////   AEC ROI    ///////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////

int ArgusV4l2Capture::setROIforAECAGC(oc::Rect roi) { return -1; }

int ArgusV4l2Capture::resetROIforAECAGC() { return -1; }
...
bool ArgusV4l2Capture::stopStream() {
    ...
    if (ioctl(ctx.cam_fd, VIDIOC_STREAMOFF, &type)) {
        printf("Failed to stop streaming: %s (%d)", strerror(errno), errno);
        return false;
    }
    return true;
}

void ArgusV4l2Capture::lock() { mut_internal.lock(); }

void ArgusV4l2Capture::unlock() { mut_internal.unlock(); }
