...
std::vector<oc::ArgusDevice> ArgusBayerCapture::getArgusDevices() {
    ...
    if (!iCameraProvider) {
        ...
        std::string endpoint =
            ...
            to_string(mTCPPort);   // Use tcp because icp cannot be shared between
                                   // sudo /no-root and the daemon must be root
...
ARGUS_STATE ArgusBayerCapture::openCamera(const ArgusCameraConfig &config, bool recreate_daemon) {
    ...
    auto iCameraProvider =
        interface_cast<ICameraProvider>(ArgusProvider::getInstance()->cameraProvider);
    if (mConfig.verbose_level > 0)
        std::cout << "[ArgusCapture] Use camera provider : " << ArgusProvider::getInstance()
                  << std::endl;
    if (!iCameraProvider) {
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] Invalid camera provider : " << ArgusProvider::getInstance()
    ...
    while (!exit_) {
        ...
        if (frame) {
            ...
            if (!iFrameLeft)
                ...
                newImage.imageTimestamp =
                    (iMetadata->getSensorTimestamp()) / 1000ULL;   // Get in us
