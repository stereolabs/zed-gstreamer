...
// sudo sed -i '/^\[Service\]$/ a Environment="enableCamInfiniteTimeout=1"'
 ...
 */
template <typename T> inline void clamp(T &x, const T min, const T max) {
    if (x < min)
        x = min;
    if (x > max)
        x = max;
...
std::vector<oc::ArgusDevice> ArgusBayerCapture::getArgusDevices() {
    ...
    auto iCameraProvider =
        interface_cast<ICameraProvider>(ArgusProvider::getInstance()->cameraProvider);
    /// If we cannot get the camera provider, argus is probably in a broken state --> communicate to
    /// the daemon for a restart
    if (!iCameraProvider) {
#if USE_ZMQ_DAEMON
        ...
        if (!(rc < 0)) {
            ...
            mTCPPort = atoi(service_file.GetValue(elems.front().c_str(), elems.back().c_str(),
                                                  std::to_string(ZMQ_DAEMON_SOCKET_PORT).c_str()));
        }
        ...
        std::string endpoint =
            ...
            to_string(mTCPPort);   // Use tcp because icp cannot be shared between sudo /no-root and
                                   // the daemon must be root
    ...
    for (int k = 0; k < devices_all->size(); k++) {
        ...
        Argus::ICameraProperties *iCameraProperties =
            Argus::interface_cast<Argus::ICameraProperties>(device_);
        ...
        const Ext::ISyncSensorCalibrationData *iSyncSensorCalibrationData =
            interface_cast<const Ext::ISyncSensorCalibrationData>(device_);
        ...
        auto iCaptureSession = UniqueObj<CaptureSession>(
            iCameraProvider->createCaptureSession(devices_all->at(k), NULL));
...
ARGUS_STATE ArgusBayerCapture::openCamera(const ArgusCameraConfig &config, bool recreate_daemon) {
    ...
    if (!iCameraProvider) {
      if (mConfig.verbose_level > 0)
        std::cerr << "[ArgusCapture] Invalid camera provider : "
                  << ArgusProvider::getInstance() << " // "
                  << ArgusProvider::getInstance()->cameraProvider << std::endl;
      ...
      // Send Message to daemon
      std::string msg =
          "ZEDX#" + std::to_string(mPort) + "#" + std::to_string(0) + "#FROZEN";
#if USE_IPC_DAEMON
    ...
    if (!h) {
        ...
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] INVALID_ARGUS_COMPONENT" << std::endl;
        return ARGUS_STATE::INVALID_ARGUS_COMPONENT;
    }
    ...
    Argus::ICameraProperties *iCameraProperties =
        Argus::interface_cast<Argus::ICameraProperties>(h->cameraDevice);
    if (!iCameraProperties) {
        ArgusProvider::changeState(mPort, ARGUS_CAMERA_STATE::OFF);
        return ARGUS_STATE::INVALID_CAMERA_PROPERTIES;
    }
    ...
    h->mCaptureSession =
        UniqueObj<CaptureSession>(iCameraProvider->createCaptureSession(h->cameraDevice, &status));
    if (!h->mCaptureSession.get()) {
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] --> Invalid Capture Session" << h->mCaptureSession.get()
    ...
    auto streamSettings =
        UniqueObj<OutputStreamSettings>(interface_cast<ICaptureSession>(h->mCaptureSession)
                                            ->createOutputStreamSettings(STREAM_TYPE_EGL));
    IOutputStreamSettings *iStreamSettings = interface_cast<IOutputStreamSettings>(streamSettings);
    IEGLOutputStreamSettings *iOutputStreamSettings =
    ...
    auto evenResolution =
        Argus::Size2D<uint32_t>(ROUND_UP_EVEN(mConfig.mWidth), ROUND_UP_EVEN(mConfig.mHeight));
    ...
    h->mStream = UniqueObj<OutputStream>(interface_cast<ICaptureSession>(h->mCaptureSession)
                                             ->createOutputStream(streamSettings.get(), &status));
    if (!h->mStream) {
        if (mConfig.verbose_level > 0)
    ...
    if (!h->mFrameConsumer.get()) {
        if (mConfig.verbose_level > 0)
            std::cout << "[ArgusCapture] --> Invalid frame consumer " << h->mFrameConsumer.get()
                      << std::endl;
        ArgusProvider::changeState(mPort, ARGUS_CAMERA_STATE::OFF);
    ...
    h->capRequest =
        UniqueObj<Request>(interface_cast<ICaptureSession>(h->mCaptureSession)->createRequest());
    if (!h->capRequest) {
        ArgusProvider::changeState(mPort, ARGUS_CAMERA_STATE::OFF);
    ...
    Argus::Status resReq =
        interface_cast<IRequest>(h->capRequest)->enableOutputStream(h->mStream.get());
    ...
    h->iSourceSettings = interface_cast<ISourceSettings>(
        interface_cast<IRequest>(h->capRequest)->getSourceSettings());
    ...
    status = isrcSettings->setFrameDurationRange(
        Argus::Range<uint64_t>(ONE_SECOND_NANOS / mConfig.mFPS, ONE_SECOND_NANOS / mConfig.mFPS));
    fps = mConfig.mFPS;

    // Get the digital ISP gain range and store it
    Argus::IAutoControlSettings *ac = Argus::interface_cast<Argus::IAutoControlSettings>(
        interface_cast<IRequest>(h->capRequest)->getAutoControlSettings());
    if (ac) {
    ...
    exposure_limits_range.second =
        std::min((unsigned long long) (ONE_SECOND_MICROS) / (fps),
                 (unsigned long long) (limitExposureTimeRange.max() /
                                       1000ULL));   // Limit if FPS is too quick for DTS max exp
                                                    // (EXPMAX = 100% of FPS as most)
    ...
    auto iOutStreamSettings = interface_cast<IStreamSettings>(
        interface_cast<IRequest>(h->capRequest)->getStreamSettings(h->mStream.get()));
    ...
    /// Create the "stream-on" request --> in Repeat mode, acquireFrame will wait until new frame is
    /// available
    Argus::Status sts =
        interface_cast<ICaptureSession>(h->mCaptureSession)->repeat(h->capRequest.get());
    ...
    if (mConfig.verbose_level > 0)
        std::cout << "[ArgusCapture] --> Using Resolution " << width << "x" << height << "@" << fps
                  << std::endl;
...
int ArgusBayerCapture::convert(void *frame, argusMonoImage &image) {
    ...
    if (fd == -1) {
        ...
        fd = iNativeBuffer->createNvBuffer(resolution, NvBufferColorFormat_ABGR32,
                                           NvBufferLayout_Pitch);
#else
        if (nChannel == 3) {
            if (!mConfig.mSwapRB)
                fd = iNativeBuffer->createNvBuffer(resolution, NVBUF_COLOR_FORMAT_RGB,
                                                   NVBUF_LAYOUT_PITCH);
            else
                fd = iNativeBuffer->createNvBuffer(resolution, NVBUF_COLOR_FORMAT_BGR,
                                                   NVBUF_LAYOUT_PITCH);

        } else if (nChannel == 4) {
            if (!mConfig.mSwapRB)
                fd = iNativeBuffer->createNvBuffer(resolution, NVBUF_COLOR_FORMAT_BGRA,
                                                   NVBUF_LAYOUT_PITCH);
            else
                fd = iNativeBuffer->createNvBuffer(resolution, NVBUF_COLOR_FORMAT_RGBA,
                                                   NVBUF_LAYOUT_PITCH);
        }
#endif
        if (fd == -1) {
...
void ArgusBayerCapture::produce() {
    ...
    if (mConfig.verbose_level > 2)
        std::cout << "[ArgusCapture][Debug] : "
                  << " Wait for connection" << std::endl;
    ...
    while (!exit_) {
        ...
        UniqueObj<Frame> frame(
            iFrameConsumer->acquireFrame(100000000, &arg_stat));   // 100ms timeout
        mut_irequest.unlock();
        if (frame) {
            ...
            if (!iFrameLeft)
                ...
                newImage.imageTimestamp = (iMetadata->getSensorTimestamp()) / 1000ULL;   // Get in
                                                                                         // us
                newImage.frameExposureTime =
                    (iMetadata->getSensorExposureTime()) / 1000ULL;   // Get in 1/100 ms
                ...
                if (mConfig.verbose_level > 3)
                    std::cout << "[ArgusCapture][Debug] --> New image available for port : "
                              << mPort << " Timestamp : " << newImage.imageTimestamp
                              << "  with settings : " << width << "x" << height << "@" << fps
                              << std::endl;
                is_running = true;
            ...
            if (is_running && !ArgusProvider::hasCameraOpening()) {
                ...
                case Argus::STATUS_CANCELLED: {
                    std::cout << "[ArgusCapture][EoF] CAM " << mPort
                              << " is frozen : " << (int) arg_stat << std::endl;
                    isfrozen_ = true;
                    usleep(100000);
                } break;
                ...
                default: {
                    ...
                    {
                        std::cout << "[ArgusCapture][Timeout] CAM " << mPort << " is frozen"
                                  << std::endl;
                        isfrozen_ = true;
                        usleep(100000);
                    }
    ...
    while (!exit_) {
        ...
        if (queue_size_ > 0) {
            ...
            if (ptr_buffer_sdk)
                ...
                std::cout
                    << "[ArgusCapture][CRITICAL] Invalid pointer provided for buffer acquisition"
                    << std::endl;
            ...
            if (s_frame_count % 60 == 0) {
                ...
                std::string msg = "ZEDX#" + std::to_string(mPort) + "#" + std::to_string(0) +
                                  "#RUN#" + std::to_string(s_frame_count);
...
int ArgusBayerCapture::getFrameExposurePercent() {
    ...
    float pp_ = 100.f * (float) (current_exp_time - exposure_limits_range.first) /
                (float) (exposure_limits_range.second - exposure_limits_range.first);
...
int ArgusBayerCapture::setFrameExposureRange(uint64_t exp_low, uint64_t exp_high) {
    ...
    if (isrcSettings) {
        if (exp_low <= 0 || exp_high <= 0) {
            ...
            status = isrcSettings->setExposureTimeRange(
                Range<uint64_t>(target_low * 1000ULL, target_high * 1000ULL));
            if (status == Argus::STATUS_OK) {
            ...
            status = isrcSettings->setExposureTimeRange(
                Range<uint64_t>((exp_low) *1000ULL, (exp_high) *1000ULL));
...
int ArgusBayerCapture::setManualExposure(int percent) {
    ...
    uint64_t time =
        exposure_limits_range.first +
        percent * float(exposure_limits_range.second - exposure_limits_range.first) / 100.f;
...
int ArgusBayerCapture::getAnalogFrameGainPercent() {
    ...
    float pp_ = 100.f * (float) (current_analog_gain - analog_gain_limits_range.first) /
                (float) (analog_gain_limits_range.second - analog_gain_limits_range.first);
    return (int) round(pp_);
}
...
int ArgusBayerCapture::setAutomaticAnalogGain() {
    int stt =
        setAnalogFrameGainRange(analog_gain_current_range.first, analog_gain_current_range.second);
    return stt;
}
...
int ArgusBayerCapture::setManualAnalogGain(int percent) {
    ...
    float real_gain_ =
        analog_gain_limits_range.first +
        percent * (analog_gain_limits_range.second - analog_gain_limits_range.first) / 100.f;
...
float ArgusBayerCapture::getDigitalFrameGain() { return current_digital_gain; }
...
int ArgusBayerCapture::setAutomaticDigitalGain() {
    int stt = setDigitalFrameGainRange(digital_gain_current_range.first,
                                       digital_gain_current_range.first);
    return stt;
}
...
int ArgusBayerCapture::setManualDigitalGain(int percent) {
    ...
    float real_gain_ =
        digital_gain_limits_range.first +
        percent * (digital_gain_limits_range.second - digital_gain_limits_range.first) / 100.f;
...
int ArgusBayerCapture::setManualWhiteBalance(uint32_t color_temperature_) {
    ...
    estimageRGGBGainFromColorTemperature_v2(color_temperature_, 1.5, whiteBalanceGains[0],
                                            whiteBalanceGains[1], whiteBalanceGains[2],
                                            whiteBalanceGains[3]);
...
int ArgusBayerCapture::setSharpening(float value) {
    ...
    Argus::IEdgeEnhanceSettings *iEdgeEnhanceSettings =
        Argus::interface_cast<Argus::IEdgeEnhanceSettings>(h->capRequest);
...
int ArgusBayerCapture::setDenoisingValue(float value) {
    ...
    Argus::Status status = Argus::STATUS_DISCONNECTED;
    Argus::IDenoiseSettings *iDenoiserSettings =
        Argus::interface_cast<Argus::IDenoiseSettings>(h->capRequest);
...
int ArgusBayerCapture::setToneMappingFromGamma(int channel, float gamma) {
    ...
    if (ac) {
        ...
        status =
            ac->setToneMapCurve(static_cast<RGBChannel>(channel), default_tone_mapping_lut_red);
        current_gamma = gamma;
    }
...
float ArgusBayerCapture::getGamma() { return current_gamma; }
...
int ArgusBayerCapture::dump_dmabuf(int dmabuf_fd, unsigned int plane, argusMonoImage &image) {
    //  if (dmabuf_fd <= 0)
    //    return -1;
    ...
    //      memcpy((unsigned char*)image.imageData,(char
    //      *)psrc_data,parm.pitch[plane]*parm.height[plane]); NvBufferMemUnMap(dmabuf_fd, plane,
    //      &psrc_data);
    //    }
    //  else
    //    {
    ...
    //      image.imageData =
    //      (uint8_t*)malloc(nChannel*parm.planes[plane].width*parm.planes[plane].height);
    ...
    //            memcpy((unsigned char*)image.imageData+i*nChannel*parm.planes[plane].width,(char
    //            *)surf._reserved+i*parm.planes[plane].pitch,nChannel*parm.planes[plane].width);
    //      }
    //      else
    //      {
    //          memcpy((unsigned char*)image.imageData,(char
    //          *)surf._reserved,parm.planes[plane].pitch*parm.planes[plane].height);
    //      }

    int c_res =
        NvBufSurface2Raw(surf, 0, 0, parm.planes[0].width, parm.planes[0].height, image.imageData);
    if (c_res != 0)
        return -3;
...
void ArgusBayerCapture::lock() { mut_internal.lock(); }

void ArgusBayerCapture::unlock() { mut_internal.unlock(); }

/// Inspired from https://tannerhelland.com/2012/09/18/convert-temperature-rgb-algorithm-code.html
void ArgusBayerCapture::estimageRGGBGainFromColorTemperature(uint32_t KelvinT, float factor,
                                                             float &r, float &g_even, float &g_odd,
                                                             float &b) {
    double tmpCalc;
...
void ArgusBayerCapture::estimageRGGBGainFromColorTemperature_v2(uint32_t KelvinT, float factor,
                                                                float &r, float &g_even,
                                                                float &g_odd, float &b) {
    int c;
    float RGB[3];
